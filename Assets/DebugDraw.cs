...
public class DebugDraw : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		for (int i = 0; i < toDisplay.Length; i++)
		{
			if (toDisplay[i] != null)
			{
				GUI.Label(new Rect(20, i * 20, 500, 20), toDisplay[i]);
			}
		}
	...
	public static void DisplayMessage(StringBuilder stringBuilder)
	{
		if (index < toDisplay.Length)
		{
			toDisplay[index] = stringBuilder.ToString();
			stringBuilder.Clear();
			index++;
		}
		else
	...
	public static void Log(string text, long start)
	{
		UnityEngine.Debug.Log(text + (Game.NanoTime() - start) / 1000000000f + "s");
	}
}
