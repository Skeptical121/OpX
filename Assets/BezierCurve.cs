...
public struct PosRotRoute : IBufferElementData
{
	public float dist;
...
public static class BezierCurve
{
	...
	public static PosRot GetMapped(this NativeArray<PosRotRoute> netDistance, float target)
	{
		int low = 0;
	...
	public static PosRotRoute[] GetRep(float3[] points, int distancePrecision)
	{
		PosRotRoute[] netDistance = new PosRotRoute[distancePrecision + 1];
		netDistance[0] = new PosRotRoute { dist = 0, posRot = new PosRot { pos = GetPos(points, 0), rot = GetRot(points, 0) } };
		for (int i = 0; i < distancePrecision; i++)
		{
			float3 thisPos = GetPos(points, (i + 1) / (float)distancePrecision);
			netDistance[i + 1] = new PosRotRoute
			{
	...
	public static Mesh CreateRenderMesh(PosRotRoute[] posList, Vector2[] offsetList, float[] uvList, float3 curveTangent, bool loop, bool hardEdges, Vector2[] normalOffsets = null, float netDistanceOffset = 0)
	{
	...
	public static Mesh CreateCylinderRenderMesh(PosRotRoute[] posList, float radius, int renderVertices)
	{
